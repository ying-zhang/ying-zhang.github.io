...
<script>
const keys = {'#subj':'全部', '#rank':'全部', '#kind':'全部'};
const prev = {'#subj': null, '#rank': null, '#kind': null};

//init
for(let k in keys) {
  let s = localStorage[k];
  if(!s) { s = '全部'; }
  document.querySelectorAll(k +' td').forEach(td => {
    td.onclick = () => update(k, td);
    if(s === td.textContent) { update(k, td); }
  });
}

function update(k, td) {
  if(td.textContent === "") return;
  td.className = "highlight";
  keys[k] = td.textContent;
  localStorage[k] = td.textContent;
  if(prev[k]){ prev[k].className = ""; }
  prev[k] = td;
  filter();
}

function filter(){
  document.querySelectorAll('#all tr').forEach(row => {
    let hit = (keys['#subj'] === row.childNodes[0].textContent || keys['#subj'] === '全部' ) &&
              (keys['#rank'] === row.childNodes[1].textContent || keys['#rank'] === '全部' ) &&
              (keys['#kind'] === row.childNodes[2].textContent || keys['#kind'] === '全部' );
    row.classList.toggle("hide", !hit);
    if(hit) { set_style(row); }
  });
}

function set_style(row){
    var seq = parseInt(row.childNodes[3].textContent);  // 序号列
    if(document.body.classList.contains('dark')){
      if(seq%2===1) { row.style.backgroundColor="#333"; } // 交替行背景色
      if(seq  ===1) { row.style.backgroundColor="#588";  row.style.color="#fff"; } // 首行背景色
      if(row.className==="a") { row.style.color="#f00"; row.style.fontWeight="bold"; } // 增
      if(row.className==="x") { row.style.backgroundColor="#369"; row.style.textDecoration="line-through"; } // 删
      if(row.className==="u") { row.style.backgroundColor="#640"; } // 升
    } else {
      if(seq%2===1) { row.style.backgroundColor="#ddd"; } // 交替行背景色
      if(seq  ===1) { row.style.backgroundColor="#7cf"; } // 首行背景色
      if(row.className==="a") { row.style.color="#f00"; row.style.fontWeight="bold"; } // 增
      if(row.className==="x") { row.style.backgroundColor="#0ff"; row.style.textDecoration="line-through"; } // 删
      if(row.className==="u") { row.style.backgroundColor="#ff0"; } // 升
    }
}

// 监控body的class变化，当.dark类被移除时刷新页面
