...
对最初由Dijkstra [4]提出并解决的并发编程问题，Knuth [1]、deBruijn [2]、Eisenberg和McGuire [3]也给出了解法。一个使用信号量的更简单的解法也已经实现[5]。这些解法在真正的多计算机系统（而不是分时多处理器系统）中使用时有一个缺点：单个单元的故障将导致整个系统停止。我们提出了一个简单的解法，它允许系统在任意单个组件失效的情况下继续运行。
