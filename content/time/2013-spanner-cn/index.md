...
<i>ACM Trans. on Comp. Sys. (TOCS)</i>, Vol. 31, No. 3, 8:1-22, Aug. 2013. <a href="https://doi.org/10.1145/2491245" target="_blank">doi.org/10.1145/2491245</a><br/>
...
Movedir是在Paxos组之间迁移目录的后台任务[14]。Movedir还用于添加或删除Paxos组的副本[26]，这是通过将组内所有数据迁移到一个有预期副本个数的新组来实现的，因为Spanner尚不支持在线更改Paxos副本数。Movedir没有作为事务来实现，这样可以避免大量数据迁移阻塞执行中的读写操作。相反，movedir记录了它将开始迁移数据的事实，并在后台迁移数据。当迁移过程仅剩特定数量的数据后，它才使用事务原子地迁移指定数量的数据，并更新双方Paxos组的元数据。
...
一次同步之后，从员的守护进程会报告一个缓慢增加的时间不确定度。$\epsilon$来自保守地使用最坏情况的本机时钟频率偏差。$\epsilon$还取决于时间主员的不确定度，以及与主员的通信延迟。在我们的生产环境中，$\epsilon$通常是时间的锯齿状函数，在每个轮询间隔内大约从1到7 ms。因此，平均误差界$\overline{\epsilon}$大部分时间为4 ms。目前，守护进程的轮询间隔为30 s，当前频率偏差设置为200 μs/s，这两者共同构成了从0到6 ms的锯齿范围。剩下的1 ms来自与主员的通信延迟。出现故障时，有可能超出锯齿边界。例如，偶尔的时间主员不可用可能导致整个数据中心范围内$\epsilon$增加。同样，过载的机器或网络链接可能会导致局部$\epsilon$突增。正确性不受$\epsilon$波动影响，因为Spanner会等待，直到离开不确定区间；但若$\epsilon$增加过多，会使性能下降。
...
在允许协调组内的副本提交之前，协调组主员要等到$\mathtt{TT.after}(s)$为真，以遵循第4.1.2节介绍的提交-等待规则。因为协调组主员基于$\mathtt{TT.now().latest}$选择$s$，并且要等到该时间戳确实已经成为过去，所以预期的等待时间至少为$2*\overline{\epsilon}$。等待时间通常与Paxos通信重叠。提交-等待后，协调组将提交时间戳发送给客户端和所有其它参与组主员。每个参与组主员都通过Paxos将事务结果写入日志。所有参与组采用相同的提交时间戳，之后释放锁。
...
关于TrueTime，必须回答两个问题：$\epsilon$是否确实反映了时钟的不确定度；$\epsilon$能有多差？对于前者，最严重的问题是本地时钟的频率偏差是否大于200 μs/s：这将打破TrueTime的假设。我们的机器统计数据显示，CPU故障的概率是时钟故障的6倍。也就是说，相对于更严重的硬件问题而言，时钟问题极少发生。因此，我们认为TrueTime的实现与Spanner所依赖的任何其它软件一样可信。

图6展示了跨2200 km的数据中心的数千台SpanServer机器上获取的TrueTime数据。它绘制了从员向时间主员获取时间之后立即采样的$\epsilon$第90%、99%和99.9%分位数。采样移除了$\epsilon$中由本机时钟不确定度产生的锯齿，因此测量的是时间主员的不确定度（通常为0）加上与时间主员的通信延迟。

<figure>
<img src="fig/6.svg" alt="Fig 6." width="600px"/>
<p><b>图6.</b> TrueTime $\epsilon$值的分布，从员向时间主员获取时间之后立即采样，画出了90%、99%和99.9%分位数.</p>
</figure>

数据表明，在确定$\epsilon$的基础值时，上述这两个因素通常不是问题。但是，存在严重的长尾延迟问题，导致的较高的$\epsilon$值。从3月30日开始减少的长尾延迟是由于网络的改进，减少了瞬时网络拥塞。4月13日较高的$\epsilon$持续了约1个小时，原因是由于数据中心的两个时间主员因例行维护而关闭。我们将继续调查并消除TrueTime波动的原因。
...
鉴于我们预计许多应用都将在相对较邻近的多个数据中心复制数据，TrueTime $\epsilon$可能会显著影响性能。我们认为把$\epsilon$值降低到1 ms以内并没有不可逾越的障碍。
...
<li>Lamport, L. 1998. The part-time parliament. TOCS. 16, 2, 133-169</li>
